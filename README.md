...
| `skip-build`                | If `true`, skip compile/test/deploy and only run Sonar analysis against `target/` output already restored on disk (e.g. downloaded from a prior build job's uploaded artifact). Requires `deploy: false`. | `false`                                                                                     |
