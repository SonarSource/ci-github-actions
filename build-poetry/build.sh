...
jfrog_poetry_install() {
  jfrog config add repox --artifactory-url "$ARTIFACTORY_URL" --access-token "$ARTIFACTORY_ACCESS_TOKEN"
  jfrog poetry-config --server-id-resolve repox --repo-resolve "$ARTIFACTORY_PYPI_REPO"
  jfrog poetry install --build-name="$PROJECT" --build-number="$BUILD_NUMBER"
}
...
jfrog_poetry_publish() {
  ...
  popd
  jfrog rt build-collect-env "$PROJECT" "$BUILD_NUMBER"
  jfrog rt build-publish "$PROJECT" "$BUILD_NUMBER" \
    --env-include 'PROJECT;GIT_*;*VERSION*;BUILD_*;GITHUB_*;*BRANCH*;*ID' \
