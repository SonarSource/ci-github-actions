...
Describe 'build-npm/build.sh'
  ...
  Describe 'Maintenance branch RELEASE version handling'
    BeforeEach 'source build-npm/build.sh'

    It 'handles maintenance branch with RELEASE version'
      export MOCK_VERSION="1.2.3"  # No SNAPSHOT suffix
      export GITHUB_REF_NAME="branch-1.2"
      export GITHUB_EVENT_NAME="push"
      export PROJECT="test-project"
      export BUILD_NUMBER="42"
      ...
      The stderr should include "WARN: Version '1.2.3' does not match the expected format '<MAJOR>.<MINOR>.<PATCH>.<BUILD_NUMBER>'"
    End
  End
End
