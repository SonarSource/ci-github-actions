...
Describe 'export_built_artifacts()'
  It 'skips silently when deployed=false'
    GITHUB_OUTPUT=$(mktemp)
    export GITHUB_OUTPUT
    mvn_output="/dev/null"
  ...
  It 'captures artifacts when deployed=true and writes to GITHUB_OUTPUT'
    GITHUB_OUTPUT=$(mktemp)
    export GITHUB_OUTPUT
    ...
    The line 1 of contents of file "$GITHUB_OUTPUT" should equal "deployed=true"
