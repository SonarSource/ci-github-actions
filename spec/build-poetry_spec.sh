...
Describe 'build.sh'
  ...
  It 'should run the main function'
    ...
    The line 11 should equal "jfrog config add repox --artifactory-url <repox url> --access-token <access token>"
    The line 12 should equal "jfrog poetry-config --server-id-resolve repox --repo-resolve <repox pypi repo>"
    The line 13 should equal "jfrog poetry install --build-name=my-repo --build-number=42"
...
Describe 'jfrog_poetry_install'
  ...
  It 'should install Poetry dependencies using JFrog CLI'
    ...
    The line 1 should include "jfrog config add repox"
    The line 2 should include "jfrog poetry-config"
    The line 3 should include "jfrog poetry install"
  End
End
...
Describe 'main'
  ...
  It 'should build and publish when it is master branch and not a PR'
    ...
    The line 2 should equal "jfrog config remove repox"
    The line 3 should equal "jfrog config add repox --artifactory-url <repox url> --access-token <deploy token>"
    The line 4 should include "/dist"
    The line 5 should equal "jfrog rt upload ./ <deploy repo>/poetry/1.0.0.42/ --module=poetry:1.0.0.42 --build-name=my-repo --build-number=42"
    # ignore line 6 with popd output
    The line 7 should equal "jfrog rt build-collect-env my-repo 42"
    The line 8 should include "jfrog rt build-publish my-repo 42"
