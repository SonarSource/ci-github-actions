...
Describe 'build_poetry()'
  ...
  It 'skips the primary platform in RUN_SHADOW_SCANS when already analyzed in PR'
    export GITHUB_EVENT_NAME="pull_request"
    export PULL_REQUEST="123"
    export GITHUB_REF_NAME="123/merge"
    export RUN_SHADOW_SCANS="true"
    ...
    The line 9 should equal 'poetry build'
    The line 10 should equal 'run_sonar_analysis()'
    The line 11 should equal '=== Running Sonar analysis on all platforms (shadow scan enabled) ==='
    The line 13 should equal '--- ORCHESTRATOR: Skipping platform next (already analyzed in PR) ---'
    The line 15 should equal '--- ORCHESTRATOR: Analyzing with platform: sqc-us ---'
    ...
    The line 19 should equal 'poetry run pysonar -Dsonar.host.url=https://sonarqube-us.com -Dsonar.token=sqc-us-token -Dsonar.analysis.buildNumber=42 -Dsonar.analysis.pipeline=dummy-run-id -Dsonar.analysis.sha1=dummy-sha -Dsonar.analysis.repository=my-org/my-repo -Dsonar.analysis.prNumber=123'
    The line 22 should equal '--- ORCHESTRATOR: Analyzing with platform: sqc-eu ---'
    The line 23 should equal 'Using Sonar platform: sqc-eu (URL: https://sonarcloud.io)'
    The line 24 should equal 'poetry run pip install pysonar'
    The line 26 should equal 'poetry run pysonar -Dsonar.host.url=https://sonarcloud.io -Dsonar.token=sqc-eu-token -Dsonar.analysis.buildNumber=42 -Dsonar.analysis.pipeline=dummy-run-id -Dsonar.analysis.sha1=dummy-sha -Dsonar.analysis.repository=my-org/my-repo -Dsonar.analysis.prNumber=123'
    The line 28 should equal '=== Completed Sonar analysis on all platforms ==='
    The status should be success
  End
